...
Start backup to `backup.img`, the `-c` option creates the file if it does not exist. If it exists, it is deleted and recreated. The size of the image corresponds to the used size of the root partition, plus additional 500Mb for /boot and 1000Mb reserve.
```
